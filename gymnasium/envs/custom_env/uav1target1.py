...
class UAV1Target1(Env):
    ...
    def __init__(
        ...
        self.observation_space = Dict(
            {  # r, alpha
                "uav1_target1": Box(
                    low=np.array([r_min, -pi]),
                    high=np.array([r_max, pi]),
                    dtype=np.float32,
                ),
        ...
        self.states = States(
            np.linspace(0.0, 80.0, self.n_r, dtype=np.float64),
            np.linspace(
                -np.pi,
        ...
        self.actions = Actions(
            np.linspace(-1.0 / 4.5, 1.0 / 4.5, self.n_u, dtype=np.float64).reshape(
                (-1, 1)
            )
        )
    ...
    def observation(self):
        dictionary_obs = {
            # r, alpha
            "uav1_target1": np.array(
                [self.rel_observation(uav=1, target=1)[0],
                self.rel_observation(uav=1, target=1)[1]],
                dtype=np.float32,
                ),
            "uav1_charge_station": np.array([self.observation1[0], self.observation1[1]], dtype=np.float32,),
            "battery":  np.array([self.battery], dtype=np.float32,),
            "age": np.array([self.age], dtype=np.float32,),
        }
