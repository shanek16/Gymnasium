...
class UAV1Target1_v2(Env):
    '''
    Verion 2: using class for uav and target: Preparing for multi uavs and multi targets.
    n(action) = 2
    '''
    ...
    def dry_step(self, action, future, discount):
        ...
        for i in range(future):
            ...
            dry_dict_observation = { # is this state s_{t+10}?: Yes it is
                ...
                "age": np.float32(target1_copy.age),
    ...
    def dict_observation(self):
        dictionary_obs = {
            ...
            "uav1_charge_station": np.array([self.uav1.obs[0], self.uav1.obs[1]], dtype=np.float32),
            "battery":  np.array([self.uav1.battery], dtype=np.float32),
            "age": np.array([self.target1.age], dtype=np.float32),
        }
